...
function update() {
  ...
  scrollSpeed = 0.6 + difficulty * 0.08;
  ...
  nextSpawn--;
  if (nextSpawn <= 0) {
    let newX = lastOutpostX + rnd(-30, 30);
    newX = clamp(newX, 12, 88);
    outposts.push({ x: newX, y: -5 });
    lastOutpostX = newX;

    if (ticks > 120 && rnd() < 0.5) {
      let gapW = clamp(30 - difficulty, 16, 30);
      let gapX = newX + rnd(-15, 15);
      gapX = clamp(gapX, gapW / 2 + 5, 100 - gapW / 2 - 5);
      obstacles.push({ y: -10, gapX: gapX, gapW: gapW });
    }
    nextSpawn = clamp(50 - difficulty * 3, 25, 50);
  }

  if (!frozen) {
    missile.y -= scrollSpeed * 0.4;
    if (missile.y < 25) missile.y = 25;
    if (missile.y > 80) missile.y = 80;
    ...
    if (targetX !== null) {
      ...
      missile.x += clamp(dx * 0.025, -0.8, 0.8);
    }
  }
