...
function buildPatterns(key, currentValue = null) {
  ...
  patterns.push(
    new RegExp(`(\\b${escapeRegex(key)}\\s*:\\s*)(-?\\d+\\.?\\d*)`, "g")
  );
  ...
  if (currentValue !== null) {
    ...
    patterns.push(
      new RegExp(`(\\b${escapeRegex(key)}\\s*[=:]\\s*)(${valueStr})\\b`, "g")
    );
  }
...
function tryLineBasedReplacement(lines, param, newValue) {
  ...
  if (param.path.includes(".")) {
    ...
    for (let i = searchStart; i < searchEnd; i++) {
      ...
      if (
        param.path === "nextObstacleTicks.initial" ||
        param.path === "nextEnemyTicks.initial"
      ) {
...
function tryHierarchicalReplacement(lines, param, newValue) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      lines[i].match(new RegExp(`\\b${escapeRegex(parentKey)}\\s*[=:]\\s*\\{`))
    ) {
      parentLineStart = i;
      break;
    }
...
function injectParameters(
  ...
  for (const param of parameterSpec) {
    ...
    if (replaced) {
      ...
      if (verbose) {
        console.log(
          `✓ Replaced ${param.path}: ${param.currentValue} → ${newValue} (${strategy})`
        );
      }
    } else {
      changes.push({
