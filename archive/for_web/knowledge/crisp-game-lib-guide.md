...
// IMPORTANT: text() only supports standard ASCII characters (0x20-0x7F)
// This includes: A-Z, a-z, 0-9, and common symbols like !"#$%&'()*+,-./:;<=>?@[\]^_`{|}~
// Non-ASCII characters (e.g., Japanese, emojis, accented letters) will NOT render correctly
text("Foo", x, y);
text(`Foo ${bar}`, 10, 10, { isSmallText: true });

// Rotation option for text() and char()
// IMPORTANT: rotation accepts INTEGER values only (not radians or degrees)
// 0 = no rotation (default), 1 = 90° clockwise, 2 = 180°, 3 = 270° clockwise
text("Score", 5, 50, { rotation: 1 }); // Rotated 90° clockwise
char("a", 50, 50, { rotation: 2 }); // Rotated 180°
